/*	g++ -o smooth smooth.cpp `pkg-config --cflags --libs opencv`
	g++ -o paralelo paralelo.cpp `pkg-config --cflags --libs opencv` -fopenmp
*/
	...
	for(int i = ini_i; i < end_i; i++) {
		for(int j = ini_j; j < end_j; j++) {
			for(int color = 0; color < 3; color++) {
				...
				average = average/25;
				out_p->at<cv::Vec3b>(i, j)[color] = (uchar)average;
			}
		}
	}
	//printf("OLAR!\n");
}
void smooth(int tipo_img) {

	//copia imagem de entrada
	if(tipo_img == 0) {
		out_p = new Mat(in.size(), CV_8U, 1);
	} else {
		out_p = new Mat(in.size(), CV_8UC3, 1);
	}
